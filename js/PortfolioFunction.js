...
;(function(window) {
	...
	function GridFx(el, options) {
		...
		this.previewDescriptionEl = this.previewEl.querySelector('.PortfolioItemPreview2');
	...
	GridFx.prototype._init = function() {
		...
		imagesLoaded( this.gridEl, function() {
			...
			new Masonry(self.gridEl, {
				itemSelector: '.gallery-inner-style2',
				isFitWidth : true
			});
	...
	GridFx.prototype._openItem = function(ev, item) {
		...
		classie.add(item, 'gallery-inner-style2--current');
		...
		var descriptionEl = item.querySelector('.ItemCaption');
		if( descriptionEl ) {
	...
	GridFx.prototype._closeItem = function() {
		...
		onEndTransition(this.originalImg, function() {
			...
			classie.remove(gridItem, 'gallery-inner-style2--current');
